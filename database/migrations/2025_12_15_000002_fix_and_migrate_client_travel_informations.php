...
return new class extends Migration
{
    ...
    {
        // Check if client_travel_informations table exists
        if (!Schema::hasTable('client_travel_informations')) {
            Log::warning('client_travel_informations table does not exist. Skipping migration.');
            return;
        }

        Log::info('Starting client_travel_informations migration...');
    ...
    protected function migrateDataFromMySQL(): void
    {
        ...
        if (!Schema::connection($sourceConnection)->hasTable('client_travel_informations')) {
            Log::info('client_travel_informations table does not exist in MySQL source. Skipping data migration.');
            return;
        }

        // Get data from MySQL source
        $sourceRecords = DB::connection($sourceConnection)
            ->table('client_travel_informations')
            ->get();
        ...
        foreach ($sourceRecords as $record) {
            try {
                ...
                if ($recordId && !$this->idExists($targetConnection, 'client_travel_informations', $recordId)) {
                ...
                if (count($batch) >= $batchSize) {
                    $result = $this->insertBatch($targetConnection, 'client_travel_informations', $batch);
                ...
                $errors++;
                Log::error("Error processing client_travel_informations record ID {$record->id}: " . $e->getMessage());
            }
        }
    ...
    /**
     * Fix PostgreSQL sequence for client_travel_informations.id
     */
    protected function fixPostgreSQLSequence(): void
    {
        Log::info('Fixing PostgreSQL sequence for client_travel_informations.id...');
        ...
        $sequenceExists = DB::selectOne("
            SELECT EXISTS (
                ...
                AND sequencename = 'client_travel_informations_id_seq'
            ) as exists
        ");

        if (!$sequenceExists->exists) {
            // Get the current max id value
            $maxId = DB::table('client_travel_informations')->max('id') ?? 0;

            // Create the sequence starting from max + 1
            DB::statement("CREATE SEQUENCE client_travel_informations_id_seq START WITH " . ($maxId + 1));
            Log::info("Created sequence starting from " . ($maxId + 1));
        } else {
            // Update sequence to be at least max(id) + 1
            $maxId = DB::table('client_travel_informations')->max('id') ?? 0;
            $currentSeqValue = DB::selectOne("SELECT last_value FROM client_travel_informations_id_seq");
            
            if ($currentSeqValue->last_value <= $maxId) {
                DB::statement("SELECT setval('client_travel_informations_id_seq', " . ($maxId + 1) . ")");
        ...
        DB::statement("
            ALTER TABLE client_travel_informations 
            ALTER COLUMN id SET DEFAULT nextval('client_travel_informations_id_seq'::regclass)
        ");

        // Set the sequence to be owned by the column
        DB::statement("
            ALTER SEQUENCE client_travel_informations_id_seq OWNED BY client_travel_informations.id
        ");
    ...
    protected function updateSequenceAfterMigration(): void
    {
        ...
        if ($maxId > 0) {
            ...
            DB::statement("SELECT setval('client_travel_informations_id_seq', {$newValue}, false)");
    ...
    {
        $dateFields = ['travel_arrival_date', 'travel_departure_date', 'created_at', 'updated_at'];
