...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (!Schema::hasTable('client_addresses')) {
            Log::warning('client_addresses table does not exist. Skipping migration.');
            return;
        }

        Log::info('Starting client_addresses migration...');
    ...
    protected function migrateDataFromMySQL(): void
    {
        ...
        if (!Schema::connection($sourceConnection)->hasTable('client_addresses')) {
            Log::info('client_addresses table does not exist in MySQL source. Skipping data migration.');
            return;
        }
        ...
        foreach ($sourceRecords as $record) {
            try {
                ...
                if ($recordId && !$this->idExists($targetConnection, 'client_addresses', $recordId)) {
                ...
                if (count($batch) >= $batchSize) {
                    $result = $this->insertBatch($targetConnection, 'client_addresses', $batch);
                ...
                $errors++;
                Log::error("Error processing client_addresses record ID {$record->id}: " . $e->getMessage());
            }
        }
    ...
    /**
     * Fix PostgreSQL sequence for client_addresses.id
     */
    protected function fixPostgreSQLSequence(): void
    {
        Log::info('Fixing PostgreSQL sequence for client_addresses.id...');
        ...
        $sequenceExists = DB::selectOne("
            SELECT EXISTS (
                ...
                AND sequencename = 'client_addresses_id_seq'
            ) as exists
        ");

        if (!$sequenceExists->exists) {
            // Get the current max id value
            $maxId = DB::table('client_addresses')->max('id') ?? 0;

            // Create the sequence starting from max + 1
            DB::statement("CREATE SEQUENCE client_addresses_id_seq START WITH " . ($maxId + 1));
            Log::info("Created sequence starting from " . ($maxId + 1));
        } else {
            // Update sequence to be at least max(id) + 1
            $maxId = DB::table('client_addresses')->max('id') ?? 0;
            $currentSeqValue = DB::selectOne("SELECT last_value FROM client_addresses_id_seq");
            
            if ($currentSeqValue->last_value <= $maxId) {
                DB::statement("SELECT setval('client_addresses_id_seq', " . ($maxId + 1) . ")");
        ...
        DB::statement("
            ...
            ALTER COLUMN id SET DEFAULT nextval('client_addresses_id_seq'::regclass)
        ");
    ...
    protected function updateSequenceAfterMigration(): void
    {
        ...
        if ($maxId > 0) {
            ...
            DB::statement("SELECT setval('client_addresses_id_seq', {$newValue}, false)");
