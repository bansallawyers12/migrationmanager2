...
class MatterOtherEmailTemplateController extends Controller
{
	...
	public function index(Request $request, $matterId = NULL)
	{
		...
		return view('Admin.feature.matterotheremailtemplate.index',compact(['lists', 'totalData', 'matter', 'matterId'])); 	
	...
	{	
		$matter = Matter::find($matterId);
		return view('Admin.feature.matterotheremailtemplate.create', compact(['matterId', 'matter']));	
	}
	...
	{		
		...
		return view('Admin.feature.matterotheremailtemplate.create');	
	}	
		...
		{		
			if(isset($templateId) && !empty($templateId))
			{
				try {
					...
					return view('Admin.feature.matterotheremailtemplate.edit', compact('fetchedData', 'matterId', 'matter'));
				} catch(Exception $e) {
