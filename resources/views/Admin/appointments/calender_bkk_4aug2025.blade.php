...
foreach($appointments as $appointment){
    ...
    if(strtotime($datetimes) >= strtotime($curtime)){
        ...
        $row['stitle'] = $addd ? addslashes($addd->client_id) : 'NA';
		//$row['name'] = base64_encode($addd->first_name.' '.$addd->last_name);

        $row['name'] = $addd ? base64_encode($addd->first_name . ' ' . $addd->last_name) : base64_encode('NA');
       ...
       // $row['full_name'] = $addd->first_name.' '.$addd->last_name;
        $row['full_name'] = $addd ? trim(($addd->first_name ?? '') . ' ' . ($addd->last_name ?? '')) : 'NA';
