...
<style>
    ...
    .xdsoft_datetimepicker .xdsoft_prev:hover, .xdsoft_datetimepicker .xdsoft_next:hover {
        background: #0056b3;
    }
    
    /* Activity Feed Width Toggle Styles */
    .activity-feed.wide-mode {
        flex: 0 0 450px !important;
        width: 450px !important;
        min-width: 450px !important;
    }
    
    .main-content.compact-mode .content-grid {
        grid-template-columns: repeat(auto-fit, minmax(400px, 1fr)) !important;
    }
    
    .main-content.compact-mode .content-grid.single-row {
        grid-template-columns: 1fr !important;
    }
    
    /* Activity Feed Header and Checkbox */
    ...
    .activity-feed-header label:hover {
        background-color: #f8f9fa;
    }
    
    #increase-activity-feed-width {
        width: 16px;
        height: 16px;
        cursor: pointer;
        margin: 0;
   ...
   .AIS-container {
    ...
    /* Convert Activity to Note Icon Styling Start*/
    .convert-activity-to-note {
        transition: all 0.3s ease;
        color: #6c757d;
    }
    
    .convert-activity-to-note:hover {
        color: #007bff;
        transform: scale(1.1);
    }
    
    .convert-activity-to-note:active {
        transform: scale(0.95);
    }
    /* Convert Activity to Note Icon Styling End */
   ...
   .notes-container {
    ...
    /* Email Handling CSS moved to separate file */
...
/* Client Portal Toggle Switch Styles */
.toggle-switch {
    position: relative;
    display: inline-block;
    width: 40px;
    ...
    <main class="main-content" id="main-content">
        ...
        <div class="tab-content" id="tab-content">
            ...
            <div class="tab-pane" id="emailhandling-tab">
                {{-- @include('Admin.email_handling') --}}  {{-- Commented out temporarily to fix 404 error --}}
                <div class="text-center p-4">
                    <h4>Email functionality temporarily disabled</h4>
                    <p>Email handling features are currently unavailable.</p>
                </div>
            </div>
...
$(document).ready(function() {
    ...
    function showClientMatterApplicationData(selectedMatter){
        ...
        $.ajax({
            url: '{{URL::to('/admin/load-application-insert-update-data')}}',
            type: 'POST',
            data: {
                client_id: clientId,
                client_matter_id: selectedMatter
...
<script>
    ...
    jQuery(document).ready(function($){
        ...
        $('#sel_matter_id_client_detail').on('change', function() {
            ...
            // Check if the URL has a matter ID (more than 7 segments for http://127.0.0.1:8000/admin/clients/detail/JSxTOFQtQzBgCmAK)
            var baseUrl;
            //('urlSegments-length='+urlSegments.length);
            if (urlSegments.length > 7) {
                // If there's a matter ID (e.g., CT_1 in .../JSxTOFQtQzBgCmAK/CT_1), remove it
                urlSegments.pop();
                baseUrl = urlSegments.join('/');
        ...
        $(document).delegate('.openapplicationdetail', 'click', function(){
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    ...
                    $('.ifapplicationdetailnot').html('<h4>Error loading client matter details. Please try again.</h4>');
                }
            });
        });
